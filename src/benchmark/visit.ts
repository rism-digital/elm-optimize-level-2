...
export type Memory = { [key: string]: boolean };
...
export const generateNodeJitLog = async (
  ...
  return exec(
    `node --allow-natives-syntax --print-opt-code --code-comments ${js} > ${jitLogPath}`
  ).then(() => {
...
const visitNode = async (
  ...
  console.log(
    label.padEnd(20, ' ') + chalk.green(' -> ') + chalk.yellow('Node')
  );
